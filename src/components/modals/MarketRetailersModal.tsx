...
interface MarketRetailersModalProps {
  marketName: string;
...
export function MarketRetailersModal({ marketName, retailers, isOpen, onClose }: MarketRetailersModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="sticky top-0 z-10 flex items-center justify-between border-b bg-white px-6 py-4">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">{marketName}</h2>
            <p className="text-sm text-gray-500">
              {retailers.length === 0
                ? 'No retailers in this market'
        ...
        <div className="p-6">
          {retailers.length === 0 ? (
            <div className="py-8 text-center text-gray-500">
              <p>No TAM retailers have been captured inside this market.</p>
            </div>
          ) : (
            <div className="space-y-6">
              {retailers.map((retailer, index) => (
                <div
                  key={retailer.id}
                  className={`${index > 0 ? 'border-t pt-4' : ''} space-y-3`}
                >
